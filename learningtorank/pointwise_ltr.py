...
def main(filtered, save_model):
    ...
    for train_index, test_index in logo.split(x, y, groups):
        ...
        if save_model:
            ...
            pickle.dump(model, open('%sLinearRegression/linearRegression_model_%s.sav' % (model_dir, q_test[0].replace('.txt','')), 'wb'))
        else:
            model = pickle.load(open('%sLinearRegression/linearRegression_model_%s.sav' % (model_dir, q_test[0].replace('.txt','')), 'rb'))
